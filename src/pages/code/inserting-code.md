...
\blurb{JuDoc makes it easy to insert code and the result of running the code; Julia code can be evaluated on the fly.}
...
In JuDoc there are two approaches that help you for this:

@@tlist
1. For Julia code, a **live-evaluation** of code blocks is supported,
1. For all languages, you can run the script separately and use JuDoc to insert the code file and/or the output generated by the code.
@@
...
If you would like the output to be re-interpeted by JuDoc as text, you can use `\textoutput` instead.
Here's an example:
...
For this, just activate the environment as you would otherwise, this will generate a `Project.toml` which will subsequently  be used by JuDoc without  you having to worry about it.

For instance, let's say that you want to use `PyCall` in some code blocks, then before starting the JuDoc server do
...
once that's done, if you now start the server, JuDoc will write
...
In other words, whenever you start the server, JuDoc will now activate the environment with that `Project.toml`.
